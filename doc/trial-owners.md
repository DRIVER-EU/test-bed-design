...
> In practice, during all Trials, it turned out to be easier to just take one of the existing Test-bed descriptions from the [Test-bed website at https://github.io/DRIVER-EU/test-bed](https://github.com/DRIVER-EU/test-bed/tree/master/docker) and adapt it manually as needed: comment out those sections that you don't need, change a port number to be in an available range, etc. An example of such a (so-called `docker-compose.yml`) file is shown below, containing a minimal Test-bed configuration with a time service and Trial Management Tool, and where the large file service is commented out (not needed). Typical Test-beds are more extensive, and contain additional tools such as the admin tool, observer support tool, schema registry UI, and several more. All these services can subsequently be run using one command `docker-compose up -d` on machines that have the free [Docker desktop software](https://www.docker.com) installed.
